...
public partial class DashboardViewModel : ObservableObject
{
    ...
    public async Task SavePatient()
    {
        ...
        var currentPatient = patientToEdit ?? new Patient();

        currentPatient.FullName = NewPatientFullName;
        currentPatient.DateOfBirth = NewPatientDateOfBirth;
        currentPatient.Gender = NewPatientGender;
        currentPatient.PhoneNumber = NewPatientPhoneNumber;
        currentPatient.Address = NewPatientAddress;
        currentPatient.Status = NewPatientStatus;
        currentPatient.Symptoms = NewPatientSymptoms;
        currentPatient.Severity = GetSeverityCode(NewPatientSeverity);

        if (string.IsNullOrEmpty(currentPatient.Id))
        {
            ...
            if (allPatients.Count > 0)
            {
                var maxId = allPatients
                    .Select(p => p.Id)
                    .Where(id => id.StartsWith("BN") && id.Length > 2) 
            ...
            currentPatient.Id = $"BN{nextNumber}";
            currentPatient.AdmittedDate = DateTime.Now;
            currentPatient.QueueOrder = WaitingQueue.Count + 1;
        }

        await _databaseService.SavePatientAsync(currentPatient); // Lưu vào database

        await LoadPatients(); // Load lại danh sách
